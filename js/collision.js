//check collision
function exitWidth(snakeX,canvas) {
    ...
    }else if (snakeX <= -10){       // check if x position is under -10 (left side)
        snakeX = canvas.width;      // set the postion to the max of the canvas = right side
    }
    return snakeX;                  // return the old or new x position of the snake
}
function exitHeight(snakeY,canvas){
    ...
    }else if (snakeY <= -10){       // check if y postion is under -10 (top side)
        snakeY = canvas.height;     // set the postion to max widht of the canvas = bottom side
    }
